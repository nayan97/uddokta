...
class SiteSettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($sitesetting){
            ...
            $simage = $request->file('icon_light');
            if($simage)
            {
                $image_name= uniqid();
                ...
                if($success)
                {
                    $sitesetting->icon_light = $image_url;
                }

            }
            $dimage = $request->file('icon_dark');
            if($dimage)
            {
                $image_name= uniqid();
                $ext = strtolower($dimage->getClientOriginalExtension());
                $image_full_name = $image_name. '.' .$ext;
                $upload_path = 'images/site-setting/';
                $image_url = $upload_path.$image_full_name;
                $success = $dimage->move($upload_path, $image_full_name);
                if($success)
                {
                    $sitesetting->icon_dark = $image_url;
                }
